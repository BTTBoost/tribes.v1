...
import {
  ...
  ERC721GatedFollowModule__factory,
...
task('full-deploy', 'deploys the entire Lens Protocol').setAction(async ({}, hre) => {
  ...
  console.log('\n\t-- Deploying nftGatedFollowModule --');
  const nftGatedFollowModule = await deployContract(
    new ERC721GatedFollowModule__factory(deployer).deploy(lensHub.address, {
      nonce: deployerNonce++,
    })
  );
  ...
  await waitForTx(
    lensHub.whitelistFollowModule(nftGatedFollowModule.address, true, { nonce: governanceNonce++ })
  );

  const isWhitelisted = await lensHub.isFollowModuleWhitelisted(nftGatedFollowModule.address);
  console.log(`is whitelisted follow module ${isWhitelisted}`);
  ...
  const addrs = {
    ...
    'nft gated follow module': nftGatedFollowModule.address,
